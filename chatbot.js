...
function postMessage(message) {
    ...
    // Make name white
    $('.user-color-item').eq(0).attr('data-color', '#607D8B').trigger('click');
    setTimeout(function(){    
        ...
        switch(command) {
            ...
            case '!ans':
                if(typeof question === 'undefined') {
                    postMessage("Please wait for a new question to be posted");
                    break;
                }
                ...
                if(answer == question.answer) {
					...
					postMessage('@' + userName + ', Wrong answer, please try again');
				}
            break;
			...
			case '!repo':
				postMessage('The current code can be found at ' + CURRENT_REPO);
			break;
...
function getBaseCommands() {
    return ['!help', '!time', '!leaderboard', '!ans', '!repo'];
}
